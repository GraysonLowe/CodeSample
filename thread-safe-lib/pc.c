...
int main(int argc, char **argv)
{
	...
	createList_mt(&pool,freeItem, poolsize);
...
void *producer(void *ptr)
{
	...
	while (i < maxcount) {
		...
		choice = random() % 2;
		if (choice) {
			addAtFront_mt(pool, item);
		} else {
			addAtRear_mt(pool, item);
		}
		i++;
	}
	pthread_exit(0);
...
void *consumer(void *ptr)
{
	...
	while (TRUE) {
		...
		choice = random() % 2;
		if (choice) {
			item = removeFront_mt(pool);
		} else {
			item = removeRear_mt(pool);
		}
		/* print item using toString method */
		if (item) {
			itemString = toStringItem(item);
